window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function production(s) {
      ...
      var money = 0, influence = 0, wages = 0;
      for (i = 0; i < era.models.length; i++) {
        var m = era.models[i];
        var lvl = mlvl(s, m.id);
        if (lvl > 0) { money += (m.income || 0) * Math.pow(UPG_INCOME, lvl - 1); influence += m.influence || 0; }
      }
      for (i = 0; i < era.staff.length; i++) {
      ...
      var grossMoney = money * (1 + (b.income || 0)) * mIncome;
      ...
      return {
        ...
        wages: wages,
        striking: striking,
        flops: flops * (1 + (b.flops || 0)) * mFlops,
    ...
    var CSS = [
      ...
      '.dc-status{flex:0 0 auto;padding:6px 12px;font-size:12px;background:#0b1410;border-bottom:1px solid #173a26;display:flex;flex-wrap:wrap;gap:4px 14px;white-space:nowrap;overflow:hidden;}',
      '.dc-status span{color:#c8ffd8;}',
    ...
    function render() {
      ...
      var era = allEra(G);
      var parts = [];
      parts.push('<span class="dc-era">' + era.name + '</span>');
      parts.push('<span>里程碑 ' + Math.round(G.milestone * 100) + '%</span>');
      parts.push('<span class="dc-money">¥' + fmt(G.money) + '</span><span class="dc-rate">(' + (P.money >= 0 ? '+' : '') + fmt(P.money) + '/s)</span>');
      if (P.wages > 0) parts.push('<span class="dc-rate">工资 -¥' + fmt(P.wages) + '/s</span>');
      if (P.striking) parts.push('<span class="dc-strike">⚠ 罢工中</span>');
      if (G.modifiers.rush && G.modifiers.rush.until > nowTs()) parts.push('<span class="dc-rush">⚡研究冲刺中</span>');
      parts.push('<span>FLOPs ' + fmt(G.flops) + '</span><span class="dc-rate">(+' + fmt(P.flops) + '/s)</span>');
      parts.push('<span>电力 ' + fmt(P.powerCap) + '/' + fmt(P.powerDemand) + ' MW</span>');
      if (P.powerFactor < 1) {
        parts.push('<span class="dc-warn">⚠ 供电不足，算力 -' + Math.round((1 - P.powerFactor) * 100) + '%</span>');
      }
      parts.push('<span>RP ' + fmt(G.rp) + '</span><span class="dc-rate">(+' + fmt(P.rp) + '/s)</span>');
      if (G.era >= 1) parts.push('<span>影响力 ' + fmt(G.influence) + '</span>');
      if (G.era >= 3) parts.push('<span>星际 ' + fmt(G.cosmic) + '</span>');
      for (var ti = 0; ti < G.training.length; ti++) {
        var trm = G.training[ti];
        var trmodel = findModel(G, trm.modelId);
        if (trmodel) {
          var trpct = Math.min(100, Math.floor(trm.progress / (trmodel.flops * P.costMul) * 100));
          parts.push('<span class="dc-train">训练中: ' + trmodel.name + ' ' + trpct + '%</span>');
        }
      }
      parts.push('<span class="dc-pp">PP ' + fmtInt(G.pp) + '</span>');
