window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      '.dc-cell-label{font-size:11px;font-weight:700;letter-spacing:1px;color:#9dffb0;background:#0f2418;border:1px solid #2a6b46;border-radius:3px;padding:1px 6px;align-self:flex-start;margin-bottom:3px;white-space:nowrap;}',
    ...
    function cmdModels(s) {
      ...
      for (var i = 0; i < era.models.length; i++) {
        ...
        if (!m.starter) {
          ...
          line += '  升级 v' + (lvl + 1) + ' 需 ' + fmt(nextCost) + ' FLOPs + ' + nextRp + ' RP';
        }
